 ...
 *
 * @version $Id: ContactList.java,v 1.2 2007-12-22 14:01:20 nichele Exp $
 */
...
implements ActionListener,
     ...
     */
    public ContactList(MainWindow mw) {
        ...
        title = new Label(ln.getString("contact_list"));
     ...
     * This method opens the modification page
     * for the selected contact if a double
     * click is detected.
     *
     * @param evt the occurred event
     */
    public void mouseClicked(MouseEvent evt) {
        if (evt.getClickCount()>=2) {
            if (butModify.isEnabled() &&
                ...
                evt.getY() <= (mw.contacts.size() * list.getPreferredSize().height / 10)) {
                mw.show(KEY_CONTACTMODIFY);
            }
        }
    }
    ...
    /**
     * Fills the list with the loaded contacts.
     * This method also disables the Modify and Delete buttons.
     */
    protected void fillList() {

        String lastName, firstName = null;
        DemoContact tmpContact = null;
        StringBuffer dn = null;

        list.removeAll();

        for (int i=0, l = mw.contacts.size(); i < l; i++) {
            tmpContact = (DemoContact)mw.contacts.elementAt(i);
            
            dn = new StringBuffer("");
                
            lastName  = tmpContact.getContact().getName()
                                  .getLastName().getPropertyValueAsString();
            firstName = tmpContact.getContact().getName()
