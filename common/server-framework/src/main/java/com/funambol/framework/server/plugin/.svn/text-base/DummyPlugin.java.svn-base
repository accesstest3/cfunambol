...
/**
 * Simple plugin that just log something
 * @version $Id: DummyPlugin.java,v 1.1 2008-06-05 06:10:37 nichele Exp $
 */
public class DummyPlugin implements Plugin {
    ...
    public void start() throws PluginException {
        if (logger.isInfoEnabled()) {
            logger.info("Starting dummy " + name + " plugin");
        }
    }

    public void stop() throws PluginException {
        if (logger.isInfoEnabled()) {
            logger.info("Stopping dummy " + name + " plugin");
        }
    }
