...
public class Task extends CalendarContent {
   ...
   //-------------------------------------------------------------- Constructors
    ...
    /**
     * Merges this task with another task.
     *
     * @param otherTask the task to merge with
     * @return the result of the merging as a MergeResult object
     */
    public MergeResult merge(Task otherTask) {

        MergeResult taskMergeResult = new MergeResult("Task");

        if (otherTask == null) {
            throw new IllegalStateException("The given Task must be not null");
        }

        super.mergeCalendarContent(taskMergeResult, otherTask);

        MergeResult result = null; // temporary object used to store the merge 
                                   // result for each field

        // dueDate
        result = PDIMergeUtils.compareProperties(this.dtEnd, otherTask.dtEnd);
        if (result.isSetARequired()) {
            this.dtEnd = otherTask.dtEnd;
        } else if (result.isSetBRequired()) {
            otherTask.dtEnd = this.dtEnd;
        }
        taskMergeResult.addMergeResult(result, "DueDate");

        // actualWork
        result = PDIMergeUtils.compareProperties(this.actualWork, otherTask.actualWork);
        if (result.isSetARequired()) {
            this.actualWork = otherTask.actualWork;
        } else if (result.isSetBRequired()) {
            otherTask.actualWork = this.actualWork;
        }
        taskMergeResult.addMergeResult(result, "ActualWork");

        // billingInformation
        result = PDIMergeUtils.compareProperties(this.billingInformation, otherTask.billingInformation);
        if (result.isSetARequired()) {
            this.billingInformation = otherTask.billingInformation;
        } else if (result.isSetBRequired()) {
            otherTask.billingInformation = this.billingInformation;
        }
        taskMergeResult.addMergeResult(result, "BillingInformation");

        // companies
        result = PDIMergeUtils.compareProperties(this.companies, otherTask.companies);
        if (result.isSetARequired()) {
            this.companies = otherTask.companies;
        } else if (result.isSetBRequired()) {
            otherTask.companies = this.companies;
        }
        taskMergeResult.addMergeResult(result, "Companies");

        // complete
        result = PDIMergeUtils.compareProperties(this.complete, otherTask.complete);
        if (result.isSetARequired()) {
            this.complete = otherTask.complete;
        } else if (result.isSetBRequired()) {
            otherTask.complete = this.complete;
        }
        taskMergeResult.addMergeResult(result, "Complete");

        // dateCompleted
        result = PDIMergeUtils.compareProperties(this.dateCompleted, otherTask.dateCompleted);
        if (result.isSetARequired()) {
            this.dateCompleted = otherTask.dateCompleted;
        } else if (result.isSetBRequired()) {
            otherTask.dateCompleted = this.dateCompleted;
        }
        taskMergeResult.addMergeResult(result, "DateCompleted");

        // owner
        result = PDIMergeUtils.compareProperties(this.owner, otherTask.owner);
        if (result.isSetARequired()) {
            this.owner = otherTask.owner;
        } else if (result.isSetBRequired()) {
            otherTask.owner = this.owner;
        }
        taskMergeResult.addMergeResult(result, "Owner");

        // percentComplete
        if ((this.complete != null) && (otherTask.percentComplete != null) &&
            ("0".equals(this.complete.getPropertyValueAsString())) &&
            (
             (this.percentComplete == null) ||
             (StringUtils.isEmpty(this.percentComplete.getPropertyValueAsString()))
            ) &&
            (HUNDRED_PERCENT.equals(otherTask.percentComplete.getPropertyValueAsString()))) {

            // In this case a "0" value is given to the percentComplete property
            // in order to avoid having the complete flag off with a completion
            // percentage = 100% after the merge.
            this.percentComplete = new Property("0");
        }
        result = PDIMergeUtils.compareProperties(this.percentComplete, otherTask.percentComplete);
        if (result.isSetARequired()) {
            this.percentComplete = otherTask.percentComplete;
        } else if (result.isSetBRequired()) {
            otherTask.percentComplete = this.percentComplete;
        }
        taskMergeResult.addMergeResult(result, "PercentComplete");

        // teamTask
        result = PDIMergeUtils.compareProperties(this.teamTask, otherTask.teamTask);
        if (result.isSetARequired()) {
            this.teamTask = otherTask.teamTask;
        } else if (result.isSetBRequired()) {
            otherTask.teamTask = this.teamTask;
        }
        taskMergeResult.addMergeResult(result, "TeamTask");

        // totalWork
        result = PDIMergeUtils.compareProperties(this.totalWork, otherTask.totalWork);
        if (result.isSetARequired()) {
            this.totalWork = otherTask.totalWork;
        } else if (result.isSetBRequired()) {
            otherTask.totalWork = this.totalWork;
        }
        taskMergeResult.addMergeResult(result, "TotalWork");


         // status
        result = PDIMergeUtils.compareProperties(this.status, otherTask.status);
        if (result.isSetARequired()) {
            this.status = otherTask.status;
        } else if (result.isSetBRequired()) {
            otherTask.status = this.status;
        }
        taskMergeResult.addMergeResult(result, "Status");
