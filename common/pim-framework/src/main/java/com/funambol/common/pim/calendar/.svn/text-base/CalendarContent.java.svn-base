...
public abstract class CalendarContent {
   ...
   /**
    ...
    /**
     * Merge this calendarContent with another calendarContent.
     * 
     * @param masterResult the MergeResult object used to store the result (its
     *                     content will be augmented with the result of this 
     *                     merge)
     * @param otherCC the calendarContent to merge with
     */
    protected void mergeCalendarContent(MergeResult masterResult, 
                                        CalendarContent otherCC ) {

        if (otherCC == null) {
            throw new IllegalStateException(
                    "The given CalendarContent must be not null");
        }
        ...
        // allDay
        result = PDIMergeUtils.compareBoolean(this.allDay, otherCC.allDay);
        if (result.isSetARequired()) {
            this.allDay = otherCC.allDay;
        } else if (result.isSetBRequired()) {
            otherCC.allDay = this.allDay;
        }
        masterResult.addMergeResult(result, "AllDay");

        // attendees
        result = PDIMergeUtils.mergeAttendeeLists(this.attendees   , 
                                                  otherCC.attendees);
        // No need to equate the attendee lists, it's already done by 
        // mergeAttendeeLists
        masterResult.addMergeResult(result, "Attendees");

        // categories
        result = PDIMergeUtils.compareProperties(this.categories   , 
                                                 otherCC.categories);
        if (result.isSetARequired()) {
            this.categories = otherCC.categories;
        } else if (result.isSetBRequired()) {
            otherCC.categories = this.categories;
        }
        masterResult.addMergeResult(result, "Categories");

        // classEvent
        result = PDIMergeUtils.compareProperties(this.accessClass   , 
                                                 otherCC.accessClass);
        if (result.isSetARequired()) {
            this.accessClass = otherCC.accessClass;
        } else if (result.isSetBRequired()) {
            otherCC.accessClass = this.accessClass;
        }
        masterResult.addMergeResult(result, "AccessClass");

        // contact
        result = PDIMergeUtils.compareProperties(this.contact   , 
                                                 otherCC.contact);
        if (result.isSetARequired()) {
            this.contact = otherCC.contact;
        } else if (result.isSetBRequired()) {
            otherCC.contact = this.contact;
        }
        masterResult.addMergeResult(result, "Contact");

        // created
        result = PDIMergeUtils.compareProperties(this.created, otherCC.created);
        if (result.isSetARequired()) {
            this.created = otherCC.created;
        } else if (result.isSetBRequired()) {
            otherCC.created = this.created;
        }
        masterResult.addMergeResult(result, "Created");

        // dalarm
        result = PDIMergeUtils.comparePropertiesWithTZ(this.dalarm   , 
                                                       otherCC.dalarm);
        if (result.isSetARequired()) {
            this.dalarm = otherCC.dalarm;
        } else if (result.isSetBRequired()) {
            otherCC.dalarm = this.dalarm;
        }
        masterResult.addMergeResult(result, "Dalarm");

        // description
        result = PDIMergeUtils.compareProperties(this.description   , 
                                                 otherCC.description);
        if (result.isSetARequired()) {
            this.description = otherCC.description;
        } else if (result.isSetBRequired()) {
            otherCC.description = this.description;
        }
        masterResult.addMergeResult(result, "Description");

        // dtStamp
        result = PDIMergeUtils.comparePropertiesWithTZ(this.dtStamp   , 
                                                       otherCC.dtStamp);
        if (result.isSetARequired()) {
            this.dtStamp = otherCC.dtStamp;
        } else if (result.isSetBRequired()) {
            otherCC.dtStamp = this.dtStamp;
        }
        masterResult.addMergeResult(result, "DtStamp");

        // dtStart
        result = PDIMergeUtils.comparePropertiesWithTZ(this.dtStart   ,
                                                       otherCC.dtStart);
        if (result.isSetARequired()) {
            this.dtStart = otherCC.dtStart;
        } else if (result.isSetBRequired()) {
            otherCC.dtStart = this.dtStart;
        }
        masterResult.addMergeResult(result, "DtStart");

        // duration
        if (this.duration == null) { // A null value means 0
            this.setDuration(new Property("0"));
        } else if (StringUtils.isEmpty(this.duration
                                           .getPropertyValueAsString())) {
            this.duration.setPropertyValue("0");
        }
        if (StringUtils.isEmpty(otherCC.duration
                                       .getPropertyValueAsString())) {
            otherCC.duration.setPropertyValue("0");
        }
        result = PDIMergeUtils.compareProperties(this.duration   , 
                                                 otherCC.duration);
        if (result.isSetARequired()) {
            this.duration = otherCC.duration;
        } else if (result.isSetBRequired()) {
            otherCC.duration = this.duration;
        }
        masterResult.addMergeResult(result, "Duration");

        // lastModified
        result = PDIMergeUtils.comparePropertiesWithTZ(this.lastModified   , 
                                                       otherCC.lastModified);
        if (result.isSetARequired()) {
            this.lastModified = otherCC.lastModified;
        } else if (result.isSetBRequired()) {
            otherCC.lastModified = this.lastModified;
        }
        masterResult.addMergeResult(result, "LastModified");

        // latitude
        result = PDIMergeUtils.compareProperties(this.latitude   , 
                                                 otherCC.latitude);
        if (result.isSetARequired()) {
            this.latitude = otherCC.latitude;
        } else if (result.isSetBRequired()) {
            otherCC.latitude = this.latitude;
        }
        masterResult.addMergeResult(result, "Latitude");

        // location
        result = PDIMergeUtils.compareProperties(this.location   , 
                                                 otherCC.location);
        if (result.isSetARequired()) {
            this.location = otherCC.location;
        } else if (result.isSetBRequired()) {
            otherCC.location = this.location;
        }
        masterResult.addMergeResult(result, "Location");

        // longitude
        result = PDIMergeUtils.compareProperties(this.longitude   , 
                                                 otherCC.longitude);
        if (result.isSetARequired()) {
            this.longitude = otherCC.longitude;
        } else if (result.isSetBRequired()) {
            otherCC.longitude = this.longitude;
        }
        masterResult.addMergeResult(result, "Longitude");

        // meetingStatus
        result = PDIMergeUtils.compareShorts(this.meetingStatus   , 
                                             otherCC.meetingStatus);
        if (result.isSetARequired()) {
            this.meetingStatus = otherCC.meetingStatus;
        } else if (result.isSetBRequired()) {
            otherCC.meetingStatus = this.meetingStatus;
        }
        masterResult.addMergeResult(result, "MeetingStatus");

        // busyStatus
        result = PDIMergeUtils.compareShorts(this.busyStatus   , 
                                             otherCC.busyStatus);
        if (result.isSetARequired()) {
            this.busyStatus = otherCC.busyStatus;
        } else if (result.isSetBRequired()) {
            otherCC.busyStatus = this.busyStatus;
        }
        masterResult.addMergeResult(result, "BusyStatus");

        // mileage
        result = PDIMergeUtils.compareIntegers(this.mileage, otherCC.mileage);
        if (result.isSetARequired()) {
            this.mileage = otherCC.mileage;
        } else if (result.isSetBRequired()) {
            otherCC.mileage = this.mileage;
        }
        masterResult.addMergeResult(result, "Mileage");

        // organizer
        result = PDIMergeUtils.compareProperties(this.organizer   ,
                                                 otherCC.organizer);
        if (result.isSetARequired()) {
            this.organizer = otherCC.organizer;
        } else if (result.isSetBRequired()) {
            otherCC.organizer = this.organizer;
        }
        masterResult.addMergeResult(result, "Organizer");

        // palarm
        result = PDIMergeUtils.comparePropertiesWithTZ(this.palarm   , 
                                                       otherCC.palarm);
        if (result.isSetARequired()) {
            this.palarm = otherCC.palarm;
        } else if (result.isSetBRequired()) {
            otherCC.palarm = this.palarm;
        }
        masterResult.addMergeResult(result, "Palarm");

        // priority
        result = PDIMergeUtils.compareProperties(this.priority   , 
                                                 otherCC.priority);
        if (result.isSetARequired()) {
            this.priority = otherCC.priority;
        } else if (result.isSetBRequired()) {
            otherCC.priority = this.priority;
        }
        masterResult.addMergeResult(result, "Priority");

        // recurrencePattern
        result = PDIMergeUtils.compareRecurrencePatterns(
                this.recurrencePattern, otherCC.recurrencePattern);
        if (result.isSetARequired()) {
            this.recurrencePattern = otherCC.recurrencePattern;
        } else if (result.isSetBRequired()) {
            otherCC.recurrencePattern = this.recurrencePattern;
        }
        masterResult.addMergeResult(result, "RecurrencePattern");

        // reminder
        result = PDIMergeUtils.compareReminders(this.reminder   , 
                                                otherCC.reminder);
        if (result.isSetARequired()) {
            this.reminder = otherCC.reminder;
        } else if (result.isSetBRequired()) {
            otherCC.reminder = this.reminder;
        }
        masterResult.addMergeResult(result, "Reminder");

        // sequence
        result = PDIMergeUtils.compareProperties(this.sequence   ,
                                                 otherCC.sequence);
        if (result.isSetARequired()) {
            this.sequence = otherCC.sequence;
        } else if (result.isSetBRequired()) {
            otherCC.sequence = this.sequence;
        }
        masterResult.addMergeResult(result, "Sequence");

        // status
        result = PDIMergeUtils.compareProperties(this.status, otherCC.status);
        if (result.isSetARequired()) {
            this.status = otherCC.status;
        } else if (result.isSetBRequired()) {
            otherCC.status = this.status;
        }
        masterResult.addMergeResult(result, "Status");

        // summary
        result = PDIMergeUtils.compareProperties(this.summary, otherCC.summary);
        if (result.isSetARequired()) {
            this.summary = otherCC.summary;
        } else if (result.isSetBRequired()) {
            otherCC.summary = this.summary;
        }
        masterResult.addMergeResult(result, "Summary");

        // uid
        result = PDIMergeUtils.compareProperties(this.uid, otherCC.uid);
        if (result.isSetARequired()) {
            this.uid = otherCC.uid;
        } else if (result.isSetBRequired()) {
            otherCC.uid = this.uid;
        }
        masterResult.addMergeResult(result, "Uid");

        // url
        result = PDIMergeUtils.compareProperties(this.url, otherCC.url);
        if (result.isSetARequired()) {
            this.url = otherCC.url;
        } else if (result.isSetBRequired()) {
            otherCC.url = this.url;
        }
        masterResult.addMergeResult(result, "Url");
