 ...
 *
 * @version $Id: Address.java,v 1.3 2007-11-28 11:14:04 nichele Exp $
 */
public class Address {
    ...
    /**
     * Merges this address with another one.
     * 
     * @param otherAddress the other address
     * @return true only if the original Address object has changed
     */
    public MergeResult merge(Address otherAddress) {

        MergeResult addressMergeResult = new MergeResult("Address");

        if (otherAddress == null) {
            throw new IllegalStateException("The given Address must not be null");
        }
        ...
        // city
        result = PDIMergeUtils.compareProperties(this.city, otherAddress.city);
        if (result.isSetARequired()) {
            this.city = otherAddress.city;
        } else if (result.isSetBRequired()) {
            otherAddress.city = this.city;
        }
        addressMergeResult.addMergeResult(result, "City");

        // country
        result = PDIMergeUtils.compareProperties(this.country, otherAddress.country);
        if (result.isSetARequired()) {
            this.country = otherAddress.country;
        } else if (result.isSetBRequired()) {
            otherAddress.country = this.country;
        }
        addressMergeResult.addMergeResult(result, "Country");


        // label
        result = PDIMergeUtils.compareProperties(this.label, otherAddress.label);
        if (result.isSetARequired()) {
            this.label = otherAddress.label;
        } else if (result.isSetBRequired()) {
            otherAddress.label = this.label;
        }
        addressMergeResult.addMergeResult(result, "Label");


        // postalCode
        result = PDIMergeUtils.compareProperties(this.postalCode, otherAddress.postalCode);
        if (result.isSetARequired()) {
            this.postalCode = otherAddress.postalCode;
        } else if (result.isSetBRequired()) {
            otherAddress.postalCode = this.postalCode;
        }
        addressMergeResult.addMergeResult(result, "PostalCode");


        // postOfficeAddress
        result = PDIMergeUtils.compareProperties(this.postOfficeAddress, otherAddress.postOfficeAddress);
        if (result.isSetARequired()) {
            this.postOfficeAddress = otherAddress.postOfficeAddress;
        } else if (result.isSetBRequired()) {
            otherAddress.postOfficeAddress = this.postOfficeAddress;
        }
        addressMergeResult.addMergeResult(result, "PostOfficeAddress");

        // extendedAddress
        result = PDIMergeUtils.compareProperties(this.extendedAddress, otherAddress.extendedAddress);
        if (result.isSetARequired()) {
            this.extendedAddress = otherAddress.extendedAddress;
        } else if (result.isSetBRequired()) {
            otherAddress.extendedAddress = this.extendedAddress;
        }
        addressMergeResult.addMergeResult(result, "ExtendedAddress");

        // roomNumber
        result = PDIMergeUtils.compareProperties(this.roomNumber, otherAddress.roomNumber);
        if (result.isSetARequired()) {
            this.roomNumber = otherAddress.roomNumber;
        } else if (result.isSetBRequired()) {
            otherAddress.roomNumber = this.roomNumber;
        }
        addressMergeResult.addMergeResult(result, "RoomNumber");

        // state
        result = PDIMergeUtils.compareProperties(this.state, otherAddress.state);
        if (result.isSetARequired()) {
            this.state = otherAddress.state;
        } else if (result.isSetBRequired()) {
            otherAddress.state = this.state;
        }
        addressMergeResult.addMergeResult(result, "State");

        // street
        result = PDIMergeUtils.compareProperties(this.street, otherAddress.street);
        if (result.isSetARequired()) {
            this.street = otherAddress.street;
        } else if (result.isSetBRequired()) {
            otherAddress.street = this.street;
        }
        addressMergeResult.addMergeResult(result, "Street");
