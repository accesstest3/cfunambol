...
extends com.funambol.common.pim.common.Property
implements MergeableWithURI {
    ...
    */
    ...
    public MergeResult merge(Attendee otherAttendee) {
        if (this.equals(otherAttendee)) {
            return new MergeResult(false, false);
        } else { // "this" prevails
            otherAttendee.name     = this.name;
            otherAttendee.uri      = this.uri;
            otherAttendee.role     = this.role;
            otherAttendee.expected = this.expected;
            otherAttendee.kind     = this.kind;
            otherAttendee.status   = this.status;
            return new MergeResult(false, true);
        }
    }

    public MergeResult merge(Object otherObject) {
        
        if (!(otherObject instanceof Attendee)) {
            return null;
        }
        return merge((Attendee) otherObject);
    }
}
