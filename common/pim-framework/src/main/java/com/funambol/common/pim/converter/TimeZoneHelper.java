...
import com.funambol.common.pim.calendar.SIFCalendar;
...
public class TimeZoneHelper {
    ...
    /**
     * Creates a new instance of TimeZoneHelper on the basis of the
     * information extracted from a SIF-E or SIF-T item.
     *
     * @param sifTimeZone the Timezone node of a SIF-E or SIF-T item
     * @throws java.lang.Exception
     */
    public TimeZoneHelper(Node sifTimeZone)
    throws Exception {
        setFormattersToUTC();
        this.name = ""; // vCalendar (1.0) has no time zone identifier because
                        // there's just one time zone per calendar item
        ...
        List<Node> sifDayLights =
                new ArrayList<Node>(timeZoneSubcomponents.getLength() - 1);
        String tz = null;
        ...
        for (Node dayLight : sifDayLights) {
            ...
            for (int i = 0; i < dayLightSubcomponents.getLength(); i++) {
                ...
                if (SIFCalendar.TIME_ZONE_DST_OFFSET.equals(child.getNodeName())) {
                    String summerOffsetString =
                            child.getTextContent().replaceAll("[\\+\\-:]", "")
                            + "00"; // More robust
                    summerOffset = 3600000 *
                            Integer.parseInt(summerOffsetString.substring(0, 2));
                    summerOffset += 60000 *
                            Integer.parseInt(summerOffsetString.substring(2, 4));
    ...
    public String getSIF() {
        ...
        for (Property property : xvCalendarProperties) {
            if ("TZ".equals(property.getName())) {
                addXMLNode(xml                              ,
                          SIFCalendar.TIME_ZONE_BASIC_OFFSET,
                          property.getValue()               );
            }
            ...
            if ("DAYLIGHT".equals(property.getName())) {
                ...
                if (!"FALSE".equalsIgnoreCase(dl)) {
                    ...
                    addXMLNode(xml                                 ,
                               SIFCalendar.TIME_ZONE_DST_OFFSET    ,
                               dlParts[1]                          );
                    addXMLNode(xml                                 ,
                               SIFCalendar.TIME_ZONE_DST_START     ,
                               dlParts[2]                          );
                    addXMLNode(xml                                 ,
                               SIFCalendar.TIME_ZONE_DST_END       ,
                               dlParts[3]                          );
                    addXMLNode(xml                                 ,
                               SIFCalendar.TIME_ZONE_DST_NAME      ,
                               (dlParts[4] == null) ?
                                   "" : dlParts[4]                 );
                    addXMLNode(xml                                 ,
                               SIFCalendar.TIME_ZONE_STANDARD_NAME ,
                               (dlParts[5] == null) ?
                                   "" : dlParts[5]                 );
