...
public class Event extends CalendarContent {
     ...
     * 
     * @param otherEvent the event to merge with
     * @return the result of the merging as a MergeResult object
     */
    public MergeResult merge(Event otherEvent) {

        MergeResult eventMergeResult = new MergeResult("Event");

        if (otherEvent == null) {
            throw new IllegalStateException("The given Event must be not null");
        }

        super.mergeCalendarContent(eventMergeResult, otherEvent);

        MergeResult result = null; // temporary object used to store the merge 
                                   // result for each field

        // dtEnd
        result = PDIMergeUtils.comparePropertiesWithTZ(this.dtEnd      , 
                                                       otherEvent.dtEnd);
        if (result.isSetARequired()) {
            this.dtEnd = otherEvent.dtEnd;
        } else if (result.isSetBRequired()) {
            otherEvent.dtEnd = this.dtEnd;
        }
        eventMergeResult.addMergeResult(result, "DtEnd");

        // replyTime
        result = PDIMergeUtils.comparePropertiesWithTZ(this.replyTime      , 
                                                       otherEvent.replyTime);
        if (result.isSetARequired()) {
            this.replyTime = otherEvent.replyTime;
        } else if (result.isSetBRequired()) {
            otherEvent.replyTime = this.replyTime;
        }
        eventMergeResult.addMergeResult(result, "ReplyTime");

        // transp
        result = PDIMergeUtils.compareProperties(this.transp      , 
                                                 otherEvent.transp);
        if (result.isSetARequired()) {
            this.transp = otherEvent.transp;
        } else if (result.isSetBRequired()) {
            otherEvent.transp = this.transp;
        }
        eventMergeResult.addMergeResult(result, "Transp");
