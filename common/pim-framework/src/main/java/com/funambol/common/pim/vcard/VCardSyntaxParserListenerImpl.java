...
public class VCardSyntaxParserListenerImpl extends AbstractVCardSyntaxParserListener {
    ...
    private int radio          = 0;
    private int telex          = 0;
    private int mobileDC       = 0;
    private int dcOnly         = 0;
    private int xOther         = 0;
                             ...
                             Token group) throws ParseException
    {
        ...
        contact.addXTag(tmpxTag, true); // many x-tags with same name allowed
    }
                         ...
                         Token group) throws ParseException
    {
        ...
        if (title == 0) {
            if (contact.getBusinessDetail().getTitles() == null) {
                contact.getBusinessDetail().setTitles(new ArrayList<Title>());
            }
                        ...
                        Token group) throws ParseException
    {
         ...
         * The first EMAIL;INTERNET;HOME;X-FUNAMBOL-INSTANTMESSENGER is
         * IMAddress, and the others are IM2Address etc.
        ...
        if ((emailMobile == 0) && (plist.getXParams().containsKey("X-CELL"))) {
            ...
            if (emailHome == 0) {
                if (contact.getPersonalDetail().getEmails() == null) {
                    contact.getPersonalDetail().setEmails(new ArrayList<Email>());
                }
            }
            ...
            if (plist.getXParams().containsKey("X-FUNAMBOL-INSTANTMESSENGER")) {
                ...
                tmpmail.setEmailType("IMAddress");
                    ...
                    tmpmail.setEmailType("IM" + (imAddress + 1) + "Address");
                }
                contact.getPersonalDetail().addEmail(tmpmail);
                imAddress++;
            } else {
            ...
            if (emailWork == 0) {
                if (contact.getBusinessDetail().getEmails() == null) {
                    contact.getBusinessDetail().setEmails(new ArrayList<Email>());
                }
            ...
            if (webPage == 0) {
                if (webPageHome == 0) {
                    if (contact.getPersonalDetail().getWebPages() == null) {
                        contact.getPersonalDetail().setWebPages(new ArrayList<WebPage>());
                    }
                }
        ...
        if (plist.containsKey("WORK")) {
            ...
            if (webPageWork == 0) {
                if (contact.getBusinessDetail().getWebPages() == null) {
                    contact.getBusinessDetail().setWebPages(new ArrayList<WebPage>());
                }
                             ...
                             Token group) throws ParseException
    {
        ...
        if (contact.getPersonalDetail().getPhones() == null) {
            contact.getPersonalDetail().setPhones(new ArrayList<Phone>());
        }
        ...
        if (telPlist.contains("WORK")) {
            ...
            // assume that only one preferred voice work telephone can exist
            ...
            if (car == 0) {
                ...
                tmphone.setPhoneType("Car" + (car + 1) + "TelephoneNumber");
            }
            contact.getPersonalDetail().addPhone(tmphone);
            car++;
            ...
            if (radio == 0) {
                ...
                tmphone.setPhoneType("Radio" + (radio + 1) + "TelephoneNumber");
            }
            contact.getPersonalDetail().addPhone(tmphone);
            radio++;
            ...
            if (telex == 0) {
                ...
                tmphone.setPhoneType("Telex" + (telex + 1) + "Number");
            }
            contact.getBusinessDetail().addPhone(tmphone);
            telex++;
            ...
            if (telPlist.contains("CELL")) {
                if (mobileDC == 0) {
                    ...
                    tmphone.setPhoneType("MobileDC" + (mobileDC + 1) + "TelephoneNumber");
                }
                mobileDC++;
            } else {
                if (dcOnly == 0) {
                    ...
                    tmphone.setPhoneType("DCOnly" + (dcOnly + 1) + "TelephoneNumber");
                }
                dcOnly++;
            }
            contact.getBusinessDetail().addPhone(tmphone);
        } else if (telPlist.contains("X-OTHER")) {
            Phone tmphone = new Phone(content);
            setParameters(tmphone, plist, group);
            if (xOther == 0) {
                tmphone.setPhoneType("OtherCustomTelephoneNumber");
            } else {
                tmphone.setPhoneType("OtherCustom" + (xOther + 1) + "TelephoneNumber");
            }
            contact.getPersonalDetail().addPhone(tmphone);
            xOther++;
        }
    ...
    public void setOrganization(String content, ParamList plist,
        ...
        if (flist.size() > pos) {
            String text = unfoldDecode(flist.getElementAt(pos), plist);
            contact.getBusinessDetail().setOfficeLocation(text);
        }
    }
    ...
    public void setLabel(String content, ParamList plist,
        ...
        Map<String, String> xParameters = plist.getXParams();
        if (xParameters.containsKey("X-FUNAMBOL-PRESERVE")
                && !(("0").equals(xParameters.get("X-FUNAMBOL-PRESERVE")))) {
            preserve = true;
        }
    ...
    public void setUid(String content, ParamList plist,
                       Token group) throws ParseException
    {
        String uid = unfoldDecodeUnescape(content, plist);
        if (plist != null) {
            String uidType = plist.getValue("TYPE");
            if (uidType != null) {
                uid = "{TYPE=" + uidType + '}' + uid;
            }
        }
        contact.setUid(uid);
    }
                          ...
                          Token group) throws ParseException
    {
        ...
        String uri = unfoldDecodeUnescape(content, plist);
        IMPPAddress impp = new IMPPAddress(uri);

        String parameters = plist.getValue("TYPE");
        if (parameters != null) {
            for (String parameter : parameters.split(",")) {

                // Type
                if ("PERSONAL".equalsIgnoreCase(parameter)) {
                    impp.setType(impp.getType() == null ?
                                 IMPPAddress.PERSONAL   :
                                 null                   );
                } else if ("BUSINESS".equalsIgnoreCase(parameter)) {
                    impp.setType(impp.getType() == null ?
                ...
                // Location
                else if ("HOME".equalsIgnoreCase(parameter)) {
                    impp.setLocation(impp.getLocation() == null ?
                                     IMPPAddress.HOME           :
                                     null                       );

                } else if ("WORK".equalsIgnoreCase(parameter)) {
                    impp.setLocation(impp.getLocation() == null ?
                                     IMPPAddress.WORK           :
                                     null                       );

                } else if ("MOBILE".equalsIgnoreCase(parameter)) {
                    impp.setLocation(impp.getLocation() == null ?
                                     IMPPAddress.MOBILE         :
                                     null                       );
                // Preferred
                } else if ("PREF".equalsIgnoreCase(parameter)) {
                    impp.setPreferred(true);
                }

                // Other values of attribute TYPE are ignored
            }
        }

        contact.addIMPPAddress(impp);
    }
    ...
    public void setProductID(String content, ParamList plist,
                             Token group) throws ParseException {
        contact.setProductID(unfoldDecodeUnescape(content, plist));
    }

    @Override
    public void setAgent(String content, ParamList plist,
                         Token group) throws ParseException {
        String value = unfoldDecodeUnescape(content, plist);
        String parameter = plist.getValue("VALUE");
        if ((parameter != null) && (parameter.equalsIgnoreCase("uri"))) {
            contact.getBusinessDetail().setAssistantURI(true);
            contact.getBusinessDetail().setAssistant(value);
        } else {
            contact.getBusinessDetail().setAssistantURI(false);
            Pattern name = Pattern.compile("\\\\[nN]FN:([^\\\\]*)\\\\[nN]");
            Matcher nameMatcher = name.matcher(value);
            if (nameMatcher.find()) {
                contact.getBusinessDetail().setAssistant(nameMatcher.group(1));
            }
            String phoneNumber = null;
            Pattern phone = Pattern.compile("\\\\[nN]TEL;(TYPE=)?WORK:([^\\\\]*)\\\\[nN]");
            Matcher phoneMatcher = phone.matcher(value);
            if (phoneMatcher.find()) { // first looks for a business phone #
                phoneNumber = phoneMatcher.group(2);
            } else { // just looks for any phone number
                phone = Pattern.compile("\\\\[nN]TEL[^:]*:([^\\\\]*)\\\\[nN]");
                phoneMatcher = phone.matcher(value);
                if (phoneMatcher.find()) {
                    phoneNumber = phoneMatcher.group(1);
                }
            }
            if (phoneNumber != null) {
                Phone assistantPhone = new Phone(phoneNumber);
                assistantPhone.setPhoneType(SIFC.ASSISTANT_TELEPHONE_NUMBER);
    ...
    public void setKey(String content, ParamList plist,
                       Token group) throws ParseException {
        contact.setKey(new Property(unfoldDecodeUnescape(content, plist)));
        setParameters(contact.getKey(), plist, group);
        if (plist != null) {
            contact.getKey().setType(plist.getValue("TYPE"));
        }
    }
     ...
     *
     * @param str the string to unfold
     ...
     * @return the string without the last equals
     */
