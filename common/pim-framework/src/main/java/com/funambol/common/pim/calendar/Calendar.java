...
public class Calendar {
    ...
    /**
     * Merges this calendar item with another calendar item.
     *
     * @param otherCalendar a Calendar object, that should be of the same type
     *                      (event/task)
     * @return a MergeResult if a merge was performed, null otherwise
     */
    public MergeResult merge(Calendar otherCalendar) {
        
        if (event != null) {
            return event.merge(otherCalendar.getEvent());
        } else if (todo != null) {
            return todo.merge(otherCalendar.getTask());
        } else { // no content
            return null;
        }
    }
