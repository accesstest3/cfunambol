...
/**
 * Represents a Suspend package sends by the client.
 *
 * The class is designed to handle the suspend alert and, if there are, possible
 * Map commands sent by the client. This is required because in a message with an
 * Alert to suspend the session, the client can't send any command but just status
 * or Mapping.
 *
 * @version $Id: SyncSuspend.java,v 1.1.1.1 2008-02-21 23:35:40 stefano_fornari Exp $
 */
public class SyncSuspend extends SyncPackage {
    ...
    private Alert suspendAlert = null;

    /**
     * Get the alert sent by the client in order to suspend the session
     * @return the alert sent by the client in order to suspend the session
     */
    ...
    public void checkBodyRequirements() throws ProtocolException {
        ...
        // check if there is an alert to suspend the session
        checkAlert();
    ...
    /**
     * Checks if in the client commands there is an alert to suspend the session
     */
    private void checkAlert() throws ProtocolException {
        suspendAlert = ProtocolUtil.getSuspendAlert(clientCommands);
        if (suspendAlert == null) {
            //
            // This case should not never happen
            //
            throw new ProtocolException("Alert with status code " + AlertCode.SUSPEND + " not found");
        }
    }
    ...
    throws ProtocolException {
        ...
        for (int i=0; i<clientCommands.length; i++) {
            ...
            if (clientCommands[i] instanceof Status) {
                continue;
            }

            if (clientCommands[i] instanceof Map) {
                targetRefs = new TargetRef[] {
                                 new TargetRef(mapCommands[i].getTarget())
                             };
                sourceRefs = new SourceRef[] {
                                 new SourceRef(mapCommands[i].getSource())
                             };
            } else if (clientCommands[i] instanceof Alert) {

                Item[] items = (Item[])((ItemizedCommand)clientCommands[i]).getItems().toArray(new Item[0]);

                List trefs = new ArrayList();
                List srefs = new ArrayList();
                Target t = null;
                Source s = null;
