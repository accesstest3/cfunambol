...
/**
 * Used to start the process to delete a SyncSouce
 *
 *
 * @version $Id: DeleteSyncSource.java,v 1.5 2007-11-28 10:28:18 nichele Exp $
 */

public class DeleteSyncSource extends ExplorerAction {
    ...
    public void performAction(Node[] nodes) {
        Log.debug("DeleteSyncSource - performAction");
        ...
        if (node instanceof SyncSourceNode) {
            sourceUri = ( (SyncSourceNode)node).getSyncSource().getSourceURI();
        } else if (node instanceof SyncSourceExceptionNode) {
            sourceUri = ( (SyncSourceExceptionNode)node).getSourceUri();
        }

        SyncTypeNode syncTypeNode = (SyncTypeNode)node.getParentNode();

        boolean syncSourceDeleted = ExplorerUtil.getSyncAdminController().getSyncSourcesController().
                                    deleteSource(
            syncTypeNode, sourceUri);
    ...
    public String getName() {
        return Bundle.getMessage(Bundle.ACTION_DELETE_NAME);
    }
