 ...
 *
 *
 * @version $Id: UsernameAccountProvider.java,v 1.1.1.1 2007-12-11 08:17:19 nichele Exp $
 */
public class UsernameAccountProvider implements IMailServerAccountProvider {
    ...
    public UsernameAccountProvider(String pushRegistryTableName){
    ...
    throws InboxListenerException, AccountNotFoundException{
        ...
        if (log.isTraceEnabled()) {
            log.trace("Starting get account for username " + username);
        }
        try {
            InboxListenerDAO dbdao = new InboxListenerDAO(pushRegistryTableName);
            msa = dbdao.getAccountByUsername(username);
            ...
            throw new InboxListenerException("Unable to load the inboxlistener " +
                    "configuration with username: " + username, ex);
        } catch (Exception ex) {
            throw new InboxListenerException("Unable to load the inboxlistener " +
                    "configuration with username: " + username, ex);
        }
        if (log.isTraceEnabled()) {
            log.trace("Account for username " + username + "done!");
        }
        return msa;
    }
