...
/**
 * @version $Id: EmailAddressParser.java,v 1.1.1.1 2007-12-11 08:17:19 nichele Exp $
 */
public class EmailAddressParser implements IUDPMessageParser {
    ...
    /** Byte sequence before data. */
    private String prefix;
    
    /** Byte sequence after data. */
    ...
    /**
     * Returns a <code>KeyAccount</code> object given a email address type message.
     * <p/>
     * The input message has the format prefix + emailaddress + postfix.
     *
    ...
    public KeyAccount parse(byte[] message) throws UDPMessageParserException {
        ...
        String emailaddress = new String(data, 0, dataLength);
        KeyAccount key = new KeyAccount();
        if (dataLength >= 1) {
            key.setMsAddress(emailaddress);
        }
        return key;
    }
}