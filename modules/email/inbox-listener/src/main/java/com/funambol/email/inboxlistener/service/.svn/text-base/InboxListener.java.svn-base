...
/**
 * Starting class for the InboxListener
 * @version $Id: InboxListener.java,v 1.3 2008-05-18 15:04:01 nichele Exp $
 */
public class InboxListener extends PushListener {

    // --------------------------------------------------------------- Constants

    private static final String POM_PROPERTIES_FILE =
            "META-INF/maven/funambol/inbox-listener/pom.properties";
    ...
    /** The logger */
    private final static Logger log = Logger.getLogger("funambol.inboxlistener");

    // ------------------------------------------------------------- Constructor
    public InboxListener() {

        super(new ServiceDescriptor("Funambol Inbox Listener",
                                    VERSION,
                                    "com.funambol.inboxlistener:type=InboxListener"));
    ...
    public static void main(String[] args) {
        ...
        String log4jFile = configPath + File.separator + "log4j-inboxlistener.xml";

        DOMConfigurator.configureAndWatch(log4jFile, 30000); // 30 sec.

        if (log.isInfoEnabled()) {
            log.info("Starting InboxListener");
        }
        ...
        try {
            ...
            log.error("Error initializing the InboxListener", ex);
            inboxListener.shutdown();
            System.exit(-1);
        }
        ...
        try {
            ...
            log.error("Error starting the InboxListener", ex);
            inboxListener.shutdown();
            System.exit(-1);
        }
