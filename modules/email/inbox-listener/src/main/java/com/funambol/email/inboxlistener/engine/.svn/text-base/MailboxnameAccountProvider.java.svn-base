 ...
 *
 *
 * @version $Id: MailboxnameAccountProvider.java,v 1.1.1.1 2007-12-11 08:17:19 nichele Exp $
 */
public class MailboxnameAccountProvider implements IMailServerAccountProvider {
    ...
    public MailboxnameAccountProvider(String pushRegistryTableName){
    ...
    throws InboxListenerException, AccountNotFoundException{
        ...
        String mailboxname = key.getMsMailboxname();

        MailServerAccount msa = null;
        if (log.isTraceEnabled()) {
            log.trace("Starting get account for mailboxname " + mailboxname);
        }
        try {
            InboxListenerDAO dbdao = new InboxListenerDAO(pushRegistryTableName);
            msa = dbdao.getAccountByMailboxname(mailboxname);
        } catch (AccountNotFoundException ex) {
            throw new AccountNotFoundException(ex);
        } catch (DataAccessException ex) {
            throw new InboxListenerException("Unable to load the inboxlistener " +
                    "configuration with mailboxname: " + mailboxname, ex);
        } catch (Exception ex) {
            throw new InboxListenerException("Unable to load the inboxlistener " +
                    "configuration with mailboxname: " + mailboxname, ex);
        }
        if (log.isTraceEnabled()) {
            log.trace("Account for mailboxname " + mailboxname + "done!");
        }
        return msa;
    }
