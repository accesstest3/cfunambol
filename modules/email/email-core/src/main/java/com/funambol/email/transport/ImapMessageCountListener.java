 ...
 *
 * <p>
 *    This class implemets a listener when a messages is added
 *    to a folder.
 * </p>
 *
 * @version $Id: ImapMessageCountListener.java,v 1.2 2008-05-13 10:09:53 gbmiglia Exp $
 */
public class ImapMessageCountListener implements MessageCountListener
{
    ...
    /**     */
    private long[] lastUIDs ;


  /**
   * Invoked when messages are added into a folder.
   *
   * @param e MessageCountEvent
   */
  public void messagesAdded(MessageCountEvent e) {
     ...
     try {
        ...
        if (msgs != null){
          int num = msgs.length;
          lastUIDs = new long[num];
          for (int i = 0; i < num; i++){
             Message msg = msgs[i];
             IMAPFolder f = (IMAPFolder)msg.getFolder();
             long uid = f.getUID(msg);
             this.lastUIDs[i] = uid;
          }
        }

        ImapEntityDAO.uid_has_been_set.setTrue();

     } catch (Exception ce){
        log.error("Error in method messagesAdded", ce);
     }
