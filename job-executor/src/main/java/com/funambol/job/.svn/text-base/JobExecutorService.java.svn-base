...
/**
 * This class is responsible to start the Job executor service
 * @version $Id: JobExecutorService.java 35046 2010-07-14 14:22:58Z pfernandez $
 */
public class JobExecutorService extends PushListener {

    // --------------------------------------------------------------- Constants

    private static final String POM_PROPERTIES_FILE =
            "META-INF/maven/funambol/job-executor/pom.properties";
    ...
    public JobExecutorService() {
        ...
        super(new ServiceDescriptor("Funambol Job Executor service",
                                    VERSION,
                                    "com.funambol.job:type=JobExecutorService"));

    }

    public static void main(String[] args) {
        String configPath = JobExecutorConfiguration.getConfigPath();

        String log4jFile = configPath + File.separator + "log4j-job-executor.xml";

        DOMConfigurator.configureAndWatch(log4jFile, 30000); // 30 sec.


        if (log.isTraceEnabled()) {
            log.trace("Starting Job Executor Service");
        }
        ...
        try {
            ...
            log.error("Error initializing the Job Executor Service", ex);
            service.shutdown();
            System.exit(-1);
        }
        ...
        try {
            ...
            log.error("Error starting the Job Executor Service", ex);
            service.shutdown();
            System.exit(-1);
        }
    }
